...
function SK5x08_MAC {
    ...
    foreach ($SK5x08_A in $SK5x08_Adps) {
        Get-ChildItem $SK5x08_Base -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -match "^\d{4}$" } | ForEach-Object {
            ...
            if ($SK5x08_D -and $SK5x08_D -eq $SK5x08_A.Name) { Set-ItemProperty -Path $_.PSPath -Name "NetworkAddress" -Value $SK5x08_NMAC.Replace(":","") -Type String }
        }
        try { Disable-NetAdapter -Name $SK5x08_A.Name -Confirm:$false -ErrorAction Stop; Start-Sleep -Seconds 2; Enable-NetAdapter -Name $SK5x08_A.Name -Confirm:$false -ErrorAction Stop } catch {}
    }
...
function SK5x08_Full {
    ...
    Write-Host "============================================================" -ForegroundColor Magenta
    SK5x08_Serial; Start-Sleep -Seconds 1
    SK5x08_MAC; Start-Sleep -Seconds 1
    ipconfig /flushdns | Out-Null; arp -d * | Out-Null; nbtstat -R | Out-Null
...
function SK5x08_Restore {
    $SK5x08_RPaths = @("HKLM:\SOFTWARE\WOW6432Node\Multi Theft Auto: San Andreas All","HKLM:\SOFTWARE\Multi Theft Auto: San Andreas All","HKCU:\SOFTWARE\Multi Theft Auto: San Andreas All","HKCU:\SOFTWARE\WOW6432Node\Multi Theft Auto: San Andreas All")
    foreach ($SK5x08_R in $SK5x08_RPaths) { if (Test-Path $SK5x08_R) { Remove-Item $SK5x08_R -Recurse -Force -ErrorAction SilentlyContinue } }
    $SK5x08_RB = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}"
    Get-ChildItem $SK5x08_RB -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -match "^\d{4}$" } | ForEach-Object { Remove-ItemProperty $_.PSPath -Name "NetworkAddress" -ErrorAction SilentlyContinue | Out-Null }
    Write-Host "[+] Restored" -ForegroundColor Green
}

function SK5x08_Status {
    $SK5x08_S = (Get-ItemProperty -Path "HKLM:\SOFTWARE\WOW6432Node\Multi Theft Auto: San Andreas All\1.5\Settings\general" -Name "cachechecksum" -ErrorAction SilentlyContinue).cachechecksum
    if ($SK5x08_S) { Write-Host "Serial: $SK5x08_S" -ForegroundColor White } else { Write-Host "No serial found" -ForegroundColor Gray }
...
do {
    ...
    if ($SK5x08_C -ne "6") { Write-Host "`nPress any key..."; $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown") }
} while ($SK5x08_C -ne "6")
