...
def take_screenshot():
    ...
    try:
        ...
        ctypes.memmove(bmi, struct.pack("<I", 40), 4)
        ctypes.memmove(ctypes.addressof(bmi) + 4, struct.pack("<i", width), 4)
        ctypes.memmove(ctypes.addressof(bmi) + 8, struct.pack("<i", height), 4)
        ctypes.memmove(ctypes.addressof(bmi) + 12, struct.pack("<HH", 1, 24), 4)
        ctypes.memmove(ctypes.addressof(bmi) + 16, struct.pack("<I", 0), 4)
        ctypes.memmove(ctypes.addressof(bmi) + 20, struct.pack("<I", image_size), 4)
        buf = ctypes.create_string_buffer(image_size)
        gdi32.GetDIBits(hdc_mem, hbmp, 0, height, buf, bmi, 0)
        bmp = b"BM" + struct.pack("<I", 54 + image_size)
