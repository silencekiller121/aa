...
def take_screenshot():
    ...
    try:
        ...
        bi = (ctypes.c_int * 10).from_buffer(bmp_info)
        bi[0] = 40
        bi[1] = width
        bi[2] = height
        bi[3] = 1 | (24 << 16)
        ...
        pixel_data = ctypes.create_string_buffer(image_size)
        
        gdi32.GetDIBits(hdc_mem, hbmp, 0, height, pixel_data, bmp_info, 0)
        
        bmp_file = bytearray()
        bmp_file.extend(b'BM')
        bmp_file.extend((54 + image_size).to_bytes(4, 'little'))
        bmp_file.extend((0).to_bytes(4, 'little'))
        bmp_file.extend((54).to_bytes(4, 'little'))
        bmp_file.extend(bmp_info)
        bmp_file.extend(pixel_data)
        
        gdi32.DeleteObject(hbmp)
        gdi32.DeleteDC(hdc_mem)
        user32.ReleaseDC(None, hdc_screen)
        
        return io.BytesIO(bytes(bmp_file))
    except Exception:
        return None
